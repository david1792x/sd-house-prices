...
## **Final remarks**
The results are **satisfactory** in accordance to the project objectives and the complexity of the designed model. This project managed to integrate very important stages in a regular **machine learning workflow**, from getting the data right from the source and creating **visualizations** to training a **machine learning** model, reporting **performance metrics** and **deploying** it for real case use. [Here](jupyter/sd-house-prices-complete.ipynb) is a link to the complete Jupyter Notebook of the project containing only the Python code associated with the first three stages for **reproducibility** purposes (make sure all of the needed files are on the same folder before running).
